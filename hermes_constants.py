"""Shared constants for Jarvis Agent.
...
def _get_platform_default_hermes_home() -> Path:
    """Return the platform-native default Jarvis home path."""
...
def get_default_hermes_root() -> Path:
    """Return the root Jarvis directory for profile-level operations.
...
def get_optional_skills_dir(default: Path | None = None) -> Path:
    ...
    tree and expose it via ``JARVIS_OPTIONAL_SKILLS``.
    """
    override = os.getenv("JARVIS_OPTIONAL_SKILLS", "").strip()
    if override:
        return Path(override)
...
def get_optional_mcps_dir(default: Path | None = None) -> Path:
    ...
    package tree and expose it via ``JARVIS_OPTIONAL_MCPS``.
    """
    override = os.getenv("JARVIS_OPTIONAL_MCPS", "").strip()
    if override:
        return Path(override)
...
def get_bundled_skills_dir(default: Path | None = None) -> Path:
    ...
    Resolution order:
        1. ``JARVIS_BUNDLED_SKILLS`` env var (Nix wrapper / explicit override)
    ...
    """
    override = os.getenv("JARVIS_BUNDLED_SKILLS", "").strip()
    if override:
        return Path(override)
...
def get_hermes_dir(new_subpath: str, old_name: str) -> Path:
    """Resolve a Jarvis subdirectory with backward compatibility.
...
def display_hermes_home() -> str:
    """Return a user-friendly display string for the current JARVIS_HOME.

    Uses ``~/`` shorthand for readability::

        default:  ``~/.jarvis``
        profile:  ``~/.jarvis/profiles/coder``
        custom:   ``/opt/jarvis-custom``

    Use this in **user-facing** print/log messages instead of hardcoding
    ``~/.jarvis``.  For code that needs a real ``Path``, use
...
def _profile_home_path(env: dict[str, str] | None = None) -> str | None:
    """Return ``{JARVIS_HOME}/home`` when the profile-home directory exists."""
    hermes_home = get_hermes_home_override() or (env or {}).get("JARVIS_HOME") or os.getenv("JARVIS_HOME")
    if not hermes_home:
        return None
...
def _iter_real_home_candidates(env: dict[str, str] | None = None) -> list[str]:
    ...
    env = env or {}
    candidates: list[str] = []
    explicit = str(env.get("JARVIS_REAL_HOME") or os.getenv("JARVIS_REAL_HOME", "")).strip()
    if explicit:
